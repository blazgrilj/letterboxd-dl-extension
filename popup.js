document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Default built-in trackers (always preserved)
  ...
  // Load all trackers (built-in + custom)
  async function loadAllTrackers() {
    const result = await chrome.storage.sync.get({
      trackers: {}
    });
    
    // Merge with defaults to ensure built-in trackers always exist
    const trackers = { ...defaultTrackers, ...result.trackers };
    
    // Ensure built-in trackers have correct data even if corrupted
    Object.keys(defaultTrackers).forEach(key => {
      if (!trackers[key] || !trackers[key].builtIn) {
        trackers[key] = { ...defaultTrackers[key] };
      }
    });
  ...
  async function saveAllTrackers(trackers) {
    await chrome.storage.sync.set({ trackers });
    notifyContentScripts(trackers);
  }

  // Notify content scripts
  async function notifyContentScripts(trackers) {
    const tabs = await chrome.tabs.query({ url: '*://letterboxd.com/*' });
    tabs.forEach(tab => {
